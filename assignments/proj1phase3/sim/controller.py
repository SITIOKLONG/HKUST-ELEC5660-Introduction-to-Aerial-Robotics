...
class Controller:
    ...
    def __call__(self, t: float, s: np.ndarray, s_des: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        # attitude control
        phi_cmd = (acc_cmd[0] * np.sin(s_rpy[2]) - acc_cmd[1] * np.cos(s_rpy[2])) / g
        theta_cmd = (acc_cmd[0] * np.cos(s_rpy[2]) + acc_cmd[1] * np.sin(s_rpy[2])) / g
        ...
        u2 = I @ (self.Kp_angle * ang_err + self.Kd_angle * ang_vel_err) + np.cross(s[10:13], I @ s[10:13])
